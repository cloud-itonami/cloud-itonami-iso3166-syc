(ns marketentry.store
  "SSoT for the Seychelles (SYC) market-entry compliance actor, behind a
  ...
  `:director-conviction-date` / `:submission-date` are ground truth for
  the `director-conviction-disqualifying?` flagship check (Public
  Procurement Act, 2008 s.90(1)(d) three-year lookback).
...
(defn demo-data
  ...
  `:director-conviction-date` / `:submission-date` are ground truth for
  the `director-conviction-disqualifying?` flagship check (s.90(1)(d)
  three-year lookback)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Mahe Marine Services Ltd" :portal "pou.gov.sc"
             ...
             :jurisdiction "SYC" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "pou.gov.sc"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :director-conviction-date nil :submission-date "2026-07-23"
             :requires-tin? true :tin-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Praslin Trading Co" :portal "pou.gov.sc"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "SYC" :status :intake}
    "eng-4" {:id "eng-4" :operator "La Digue Export Ltd" :portal "pou.gov.sc"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "SYC" :status :intake}
    "eng-5" {:id "eng-5" :operator "Victoria Logistics Group" :portal "pou.gov.sc"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :director-conviction-date nil :submission-date "2026-07-23"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               director-conviction-date submission-date
  ...
  (cond-> {:engagement/id id}
    ...
    director-conviction-date              (assoc :engagement/director-conviction-date director-conviction-date)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/director-conviction-date :engagement/submission-date
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :director-conviction-date (:engagement/director-conviction-date m)
