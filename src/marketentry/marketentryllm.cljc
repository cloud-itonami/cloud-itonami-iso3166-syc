(ns marketentry.marketentryllm
  ...
  the Seychelles (SYC) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world pou.gov.sc portal
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "director-conviction-date=" (:director-conviction-date e)
