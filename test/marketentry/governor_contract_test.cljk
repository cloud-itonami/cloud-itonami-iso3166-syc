  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Mahe Marine Services Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Mahe Marine Services Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest director-conviction-disqualifying-is-held-and-unoverridable
  (testing "director/officer conviction still within the s.90(1)(d) three-year lookback -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:director-conviction-disqualifying} (-> (store/ledger db) last :basis)))
